...
from .forms import CoinGameForm
from blog.utils import is_ajax
def pheonix(request):
    return render(request,'blog/pheonix.html')

def index(request):
    forms = [CoinGameForm(prefix=str(i)) for i in range(4)]
...
def play_game(request):
    if request.method == 'POST':
        ...
        for i in range(4):
            form = CoinGameForm(request.POST, prefix=str(i))
            if form.is_valid():
                flips.append(int(form.cleaned_data['flips']))
                piles.append(int(form.cleaned_data['piles']))
            else:
        ...
        for i in range(4):
            flip = flips[i]
            pile_choice = piles[i]
            piles_data[pile_choice].append(flip ^ coins[i])

        if len(piles_data[0]) == len(piles_data[1]):
            head1 = sum(piles_data[0])
            head2 = sum(piles_data[1])
            if head1 == head2:
